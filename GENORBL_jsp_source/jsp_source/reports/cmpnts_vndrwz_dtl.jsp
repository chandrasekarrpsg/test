...
<input id="img1" type="image" alt="EXPORT TO EXCEL" src="excel_img.jpg"
 onClick="window.open('cmpnts_vndrwz_dtl_xls.jsp?gstn=<%=genstn%>&dept=<%=dept%>&frm_dt=<%=frm_dt%>&to_dt=<%=to_dt%>&basis=<%=basis%>&vnd_id=<%=vnd_id%>&vnd_nm=<%=vnd_nm%>');" >
...
<TD align=left>
    <input type = "button" value="BACK" onclick="window.parent.open('bill_cmpnts_vndrwz.jsp?userid=<%=sLoginEmpCd%>','_self');">
</TD>
</TABLE>
...
<tr>
    ...
    <td align=center colspan="13"><font color="#400040" size="3"><b><%=disp_basis%></b></font></td>
</tr>
<tr >
...
<table width="100%" style="border: 1px solid black"; class="rcrds">
     ...
     <tr bgcolor="#DBF2DC">
          ...
          <td align="center"  colspan="7" class= "rcrdsEle"><font size=2 color="#400040"><b>Bill Components (in Rs.)</b></font></td>
     </tr>
...
job_desc2="replace(replace(replace(replace(replace(nvl(B.DESCRIP,' '),'\\','\\\\'),'\"','&quot;'),'<','&lt;'),chr(10),'\n'),'>','&gt;')";
        ...
        qry_str = "SELECT DISTINCT "
                ...
                + "          TO_CHAR(NVL(B.MATERIAL,0),'999,99,99,990.99') MTRL,              "
                + "          TO_CHAR(NVL(B.LABOUR,0),'999,99,99,990.99')  LBR,                "
                + "          TO_CHAR(NVL(B.SERVICE_TAX,0),'999,99,99,990.99') S_TAX,          "
                + "          TO_CHAR(NVL(B.VAT,0),'999,99,99,990.99') VAT,                    "
                + "          TO_CHAR(NVL(B.OT,0),'999,99,99,990.99') OVR_TM,                  "
                + "          TO_CHAR(NVL(B.OTHER_AMOUNT,0),'999,99,99,990.99') OTHR,          "
                ...
                + "       TO_CHAR(B.BILL_RELEASE_DATE,'DD-MON-YYYY') RLZ_DT,            "
                + "       NVL(TRIM(TO_CHAR(B.FINANCE_RECVD_DATE,'DD-MON-YYYY')),'NP') FIN_RCV, "
                + "       TO_CHAR(0,'999,99,99,990.99') MTRL,                              "
                + "       TO_CHAR(0,'999,99,99,990.99') LBR,                               "
                + "       TO_CHAR(0,'999,99,99,990.99') S_TAX,                             "
                + "       TO_CHAR(0,'999,99,99,990.99') VAT,                               "
                + "       TO_CHAR(0,'999,99,99,990.99') OVR_TM,                            "
                + "       TO_CHAR(0,'999,99,99,990.99') OTHR,                              "
...
<% } %>
         ...
         <td align="left" class= "rcrdsEle" ><font size=1 color="#400040"><p title='<%=cert_by%>'><%=rlzd_dt%></p></font></td>
