...
String vnd_cd    = request.getParameter("vnd_id");
...
<input id="img1" type="image" alt="EXPORT TO EXCEL" src="excel_img.jpg"
 onClick="window.open('orderwz_dtl_xls.jsp?gstn=<%=genstn%>&dept=<%=dept1%>&frm_dt=<%=st_dt1%>&to_dt=<%=end_dt1%>&basis=<%=basis%>&vnd_cd=<%=vnd_cd%>&vnd_nm=<%=vnd_nm%>&ord_tp=<%=ord_tp%>&ord_desc=<%=ord_dsc%>');" >
...
<TD align=left>
    <input type = "button" value="BACK" onclick="window.parent.open('bill_order_vndrwz.jsp?userid=<%=sLoginEmpCd%>','_self');">
</TD>
</TABLE>
...
if(basis.equals("B")){
        disp_basis = " BILLS RELEASED FOR "+vnd_nm+" BY "+genstn2+" FOR "+ord_desc+" ORDER ON THE BASIS OF BILL RELEASED AMOUNT";
}else{
        disp_basis = " BILLS RELEASED FOR "+vnd_nm+" BY "+genstn2+" FOR "+ord_desc+" ORDER ON THE BASIS OF FINANCE RECEIVED AMOUNT";
}
...
job_dsc="replace(replace(replace(replace(replace(nvl(O.JOB_DESC,' '),'\\','\\\\'),'\"','&quot;'),'<','&lt;'),chr(10),'\n'),'>','&gt;')";
           ...
           qry_str=" SELECT DISTINCT                                                              "  
	          ...
	          +" TO_CHAR(NVL(B.BILL_AMOUNT,0),'9,99,99,99,99,990.99') BILL_AMT                " 
