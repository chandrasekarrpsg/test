...
<%  response.setHeader("Content-Disposition", "attachment; filename=bill_issuing_delay.xls");%>
...
<title>BILL RELEASING DELAY</title>
...
if(st_dt1!= null && end_dt1!= null && dept1 != null){
   ...
   String bill_rcv_dt   = new String();
   String bill_rlz_dt   = new String();
   String bill_rlz_by   = new String();
...
<tr>
<td align=center colspan="8"> <font color="#400040" size="3"><b>DELAY (DAYS) IN RELEASING OF BILLS</b></font></td>
</tr>
<tr >
<td align=center colspan="8"><font color="#400040" size="3"><b>BETWEEN <%=st_dt1%> AND <%=end_dt1%></b></font></td>
</tr>
</table>
...
<tr bgcolor="#DBF2DC">
    <td colspan="8" align="center" class= "rcrdsEle"><font color="#400040" size="2">
...
<tr bgcolor="#DBF2DC">
          ...
          <td align="center"  colspan="4" class= "rcrdsEle"><font size=2 color="#400040"><b>Bill Details</b></font></td>
...
<%
        ...
        if(!genstn.equals("99999") && !dept1.equals("999")){
            ...
            whr_cls = "WHERE  B.station = ?                         "
                    + "AND    O.dept    = ?                         "
                    + "AND    B.BILL_DATE BETWEEN to_date(?,'DD-MON-YYYY') AND to_date(?,'DD-MON-YYYY')"
           ...
           whr_cls =",ORBL_CD_DEPT   D                              "
            ...
            whr_cls=" WHERE  B.BILL_DATE BETWEEN to_date(?,'DD-MON-YYYY') AND to_date(?,'DD-MON-YYYY')"
                   +" AND    O.ORDER_ID = B.ORDER_ID               "
                   +" AND    B.BILL_CERTIFIED_BY = A.EMP_CODE(+)            ";
...
qry_str = "SELECT TRIM(NVL(O.ORDER_NO,'NP')) Order_nO,                                                      "
        ...
        + "       NVL(tO_char(B.BILL_RECEIVED_DATE,'DD-MON-YYYY'),' ') RCVD_ON,                             "
        + "       NVL(tO_char(B.BILL_RELEASE_DATE,'DD-MON-YYYY'),' ')  RLSD_ON,                             "
        + "       TRIM(NVL(A.EMP_FNAME,' '))||' '||TRIM(NVL(A.EMP_LNAME,' ')) Rlsd_By,                      "
        + "       NVL(tO_char(trunc(B.BILL_RELEASE_DATE) - trunc(B.BILL_RECEIVED_DATE)),'0')  NO_OF_DELAY   "
        ...
        bill_rcv_dt   = rs.getString(5);
        bill_rlz_dt   = rs.getString(6);
        bill_rlz_by   = rs.getString(7);
...
<% }        %>
    ...
    <td align="center" class= "rcrdsEle" ><font size=1 color="#400040"><%=bill_rcv_dt%></font></td>
    <td align="center" class= "rcrdsEle" ><font size=1 color="#400040"><%=bill_rlz_dt%></font></td>
    <td align="left"   class= "rcrdsEle" ><font size=1 color="#400040"><%=bill_rlz_by%></font></td>
