...
//alert(parameter);
window.open("bgtm_repo7a.jsp?"+parameter,"_self");
}
...
%>
<form method="post" name="f1" action="bgtm_repo7a.jsp">
...
<%
       ps = mk.con.prepareStatement(" select a.gr_cd,bgt_hd,gr_desc,decode(nvl(contr_matr_mkr,'@'),'M','Materials','C','Contracts',nvl(contr_matr_mkr,'@'))  "+
				    ...
				    " where a.gr_cd=b.gr_cd "+
                                    " and a.BGT_HD IN ('7f','7e','8','9a','9b','10d','14','15','16','20c','20d','20e','20g')"+
...
%>
                <td style="width: 10%" class="<%=sStyle2%>"><strong><a href="#" onClick="window.open('view_act_exp_dtl.jsp?finyr1=<%=finyr1%>&month=<%=month%>&station=<%=station%>&dept=<%=section%>&finyr=<%=finyr%>&bgt_hd=<%=rs.getString(2)%>&empcd=<%=empcd%>&view_mkr=M&mon_basis=<%=mon_basis%>','_new');"><%=mtd_exp_act%></strong></td>
<%
}else{
%>
...
%>
                <td style="width: 10%" class="<%=sStyle2%>"><strong><a href="#" onClick="window.open('view_act_exp_dtl.jsp?finyr1=<%=finyr1%>&month=<%=month%>&station=<%=station%>&dept=<%=section%>&finyr=<%=finyr%>&bgt_hd=<%=rs.getString(2)%>&empcd=<%=empcd%>&view_mkr=Y&mon_basis=<%=mon_basis%>','_new');"><%=ytd_exp_act%></strong></td>
<%
}else{
%>
...
if(group.equals("ALL")){
       ...
       ps1 = mk.con.prepareStatement(" select /*+ rule */ to_char(nvl(sum(amt),0),'9999999990.999') "+
                                     " from BGTM_REPO_STRUC_SPL "+
                                     " where BGT_FROM = ? "+ 
                                     " and   BGT_HD IN ('7f','7e','8','9a','9b','10d','14','15','16','20c','20d','20e','20g') "+
                                     " and   bgt_mkr='B' "+
                                     " and   target_floor ='T' "+
                                     " and   STN_CD like decode(?,'BBGS','B','TGS','T','SGS','S','NCGS','N','CTM','C','EDGE','E','ALL','%') "+ 
                                     " and   SEC_CD like decode(?,'ALL','%',?) "+
...
%>
        <tr>
        <tr>
                <td style="width: 30%" class="style9"><strong>CONTROLLABLE O&M EXPENSES</strong></td>
                <td style="width: 10%" class="style9"><strong>&nbsp;</strong></td>
