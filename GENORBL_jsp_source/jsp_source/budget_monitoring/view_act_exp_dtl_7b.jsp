...
try{
     ...
     cur_yr    = request.getParameter("cur_yr");
     frm_month = request.getParameter("frm_month");
     frm_yr    = request.getParameter("frm_yr");
	 ...
	 cur_month = request.getParameter("cur_month");
...
if(cur_month_C.equals("Apr")){
       ...
       ps = mk.con.prepareStatement(" select DECODE(count(*),1,'Mar') " + 
...
if(cur_month_M.equals("Apr")){
	   ...
	   ps = mk.con.prepareStatement(" select DECODE(count(*),1,'Mar')  from FINNANCE_FINYR_FOR_M where FINANCE_FINYR = " + 
                                    " and STN = decode('BBGS','BBGS','B','SGS','S','TGS','T','NCGS','N','CTM','C','EDGE','E','B') "+
                                    " and end_date >= to_date(?,'dd/mm/rr') and substr(?,8,4) <> substr(?,1,4) ");
       ps.setString(1,finyr);
       ps.setString(2,station);
...
if(cont_matr_desc.equals("Materials")){
  if(view_mkr.equals("M")){
     month_act=cur_month_M;
  }else{
     month_act="Apr - "+cur_month_M;
  }
}else{
  if(view_mkr.equals("M")){
     month_act=cur_month_C;
  }else{
     month_act="Apr - "+cur_month_C;
  }
...
if(view_mkr.equals("M")){
       ...
       ps.setString(4,dept);
       ps.setString(5,dept);
       ps.setString(6,cur_month_M);
       ps.setString(7,finyr1);
       ps.setString(8,todt_M);
       ps.setString(9,bgt_hd);
       ps.setString(10,bgt_hd);
       ps.setString(11,mon_basis);
       rs = ps.executeQuery();
}else{
       ps = mk.con.prepareStatement(" select stn_name,DEPT,BUDGET_HD,nvl(description,'&nbsp'),nvl(QTY,'&nbsp'),nvl(UNIT_RATE,'&nbsp'),to_char(TRANS_AMT,'99999999990.999') "+ 
                                    " from BGTM_ACT_EXP_VIEW_SPL "+
       ...
       ps.setString(8,frmdt_M);
       ps.setString(9,todt_M);
       ps.setString(10,mon_basis);
       rs = ps.executeQuery();
}
%>
<table style="width: 100%" border="1">
...
if(view_mkr.equals("M")){
       ...
       ps.setString(2,station);
       ps.setString(3,station);
       ps.setString(4,cur_month_C);
       ps.setString(5,finyr1);
       ps.setString(6,todt_C);
       ps.setString(7,dept);
       ps.setString(8,dept);
       ps.setString(9,bgt_hd);
       ps.setString(10,bgt_hd);
       ...
       ps.setString(6,bgt_hd);
       ps.setString(7,bgt_hd);
       ps.setString(8,frmdt_C);
       ps.setString(9,todt_C);
       ps.setString(10,mon_basis);
       rs = ps.executeQuery();
}
%>
