...
<br>
<%
       ps = mk.con.prepareStatement(" select count(*),to_char(nvl(sum(billval_net),0),'9999999990.99')  "+
...
<%
       ps = mk.con.prepareStatement(" select ORDER_DATE,ORDER_TO,ORDER_VAL,BILL_NO,BILL_DATE,to_char(BILLVAL_NET,'9999999990.99'),BILL_RELEASE_DATE,ORDER_FOR,dept,rec_id,nvl(budget_head,'Select') "+
                                    " from IL_RM_STNORDERS_CONSTRUCTION "+
                                    " where to_char(to_date(s_month,'MM'),'Mon')=? "+
                                    " and finyear=? "+
                                    " and  station=?  "+
...
%>
        <tr>
           ...
           <select size="1" name="bgthd<%=cnt%>" style="font-family: Tahoma; font-size: 8pt; color: #800080; font-weight: bold" onChange=""/>
              <option selected value=<%=rs.getString(11)%>><%=rs.getString(11)%></option>
<%
if(!rs.getString(11).equals("Select")){
%>
...
<TABLE height=20% width="100%" border=0 cellspacing="0">
        <TBODY>
          <TR>
            <TD align=left width=100% height=100% ><font color="blue" face="Courier New" Size=4><b>Total No of Cases: <%=cnt_flg%> &nbsp;&nbsp;&nbsp; Total Bill Value Net (Rs.) : <%=tot_amt%></b></font></TD>
          </TR>
        </TBODY>
</TABLE>
<br>
