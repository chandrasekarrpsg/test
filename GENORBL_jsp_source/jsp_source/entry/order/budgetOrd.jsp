...
#menu li { background: #ffffff; border-right: 1px solid #fff; position: relative; float: left; white-space: nowrap }
...
<body bgcolor="ghostwhite" onload = '' background="Pics/Order/lblue047.gif"  >
  ...
  MakeConnectionGENORBL mk = new MakeConnectionGENORBL();
    ...
    try {
		...
		ResultSet rs6 = null ;
                ...
                String sLov = request.getParameter("sLov");

String itmQry = " select distinct oh.MAIN_MENU , oh.GROUP_NO  from ORBL_HEADS oh WHERE lov_for LIKE ? order by oh.GROUP_NO ";

String itmBdgtQry = " select  oh.BUDGET_HEAD  from ORBL_HEADS oh where  oh.MAIN_MENU = ? AND lov_for LIKE ? ";

String rowQry = " select distinct oh.SERC_HEAD , oh.SEQ_NO  from ORBL_HEADS oh where oh.MAIN_MENU = ? and oh.SERC_HEAD is not null AND lov_for LIKE ?  order by oh.SEQ_NO ";

String rowBdgtQry = " select oh.seq_no , oh.BUDGET_HEAD , REPLACE(REPLACE(oh.sub_menu,chr(39),chr(92)||chr(39)),'\"',chr(92)||chr(39)) from ORBL_HEADS oh where  oh.MAIN_MENU = ? AND lov_for LIKE ?   ";

String colQry = " select distinct oh.SUB_MENU , oh.SUB_ACCOUNT  from ORBL_HEADS oh  where oh.MAIN_MENU = ?  and  oh.SUB_MENU  is not null AND lov_for LIKE ?  order by oh.SUB_ACCOUNT ";
String colBdgtQry = " select oh.seq_no , oh.BUDGET_HEAD , REPLACE(REPLACE(oh.sub_menu,chr(39),chr(92)||chr(39)),'\"',chr(92)||chr(39)) from ORBL_HEADS oh where  oh.MAIN_MENU = ? and  oh.SUB_MENU = ? AND lov_for LIKE ?  ";
...
while(rs.next()){
					...
					while(rs3.next()){
						...
						<%
							...
							ps6.setString(2,rs3.getString(1));
                                                        ps6.setString(3,"%"+sLov+"%");
							rs6 = ps6.executeQuery();
							if(rs6.next()){
							%>
								<li><a href="#" onclick="populate('<%= rs6.getString(1) %>|<%= rs6.getString(2) %>|<%= rs6.getString(3) %>');" ><%= rs3.getString(1) %></a></li>
							<%
							}
							rs6.close();
							ps6.close();
				...
				if(!colExist){
					...
					ps5.setString(1,rs.getString(1));
                                        ps5.setString(2,"%"+sLov+"%");
					rs5 = ps5.executeQuery();
					if(rs5.next()){
					%>
						<li><a href="#" onclick="populate('<%= rs5.getString(1) %>|<%= rs5.getString(2) %>|<%= rs5.getString(3) %>');" ><%= rs.getString(1) %></a></li>
					<%
					}
					rs5.close();
					ps5.close();
