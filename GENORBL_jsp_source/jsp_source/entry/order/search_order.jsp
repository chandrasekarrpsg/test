<%--
    ...
    Created on : Dec 4, 2012, 2:08:34 PM
    Author     : debasis saha
--%>
...
<head>
    ...
    <script>
        ...
        function getOrdSts(){
            ...
            var ordStsXML ;
            try{
                ordStsXML = new ActiveXObject("Msxml2.XMLHTTP");
            }catch(e){
                try{
                    ordStsXML = new ActiveXObject("Microsoft.XMLHTTP");
                }catch(e){
                    ordStsXML = new XMLHttpRequest();
                }
            }
            ordStsXML.open('POST',url,true);
            ordStsXML.send('');
            ordStsXML.onreadystatechange = function(){
                if(ordStsXML.readyState == 4){
                    //alert(equipXML.responseText);
                    myval = ordStsXML.responseXML.getElementsByTagName("ORDSTS");

                    for(i = document.f1.sOrdStatus.options.length - 1 ; i > 0 ; i--){
                        document.f1.sOrdStatus.remove(i) ;
                    }
                    ...
                    for(var i = 0 ; i < myval.length ; i++){
                        ...
                        //alert(cell[0]+"---"+cell[1]);
                        option = new Option(cell[1],cell[0]) ;
                        document.f1.sOrdStatus.add(option) ;
...
<body background="Pics/Order/lblue047.gif" onload="menuDisplay();">
    ...
    <b>
        ...
        <br><br-->
        <form name="f1" action="order_list.jsp" method="post" onsubmit="return validate_form();">
                        ...
                        <tr>
                            ...
                            <td colspan="4">
                                <b><font face="Times New Roman" color="#FFFFFF">
                                        <select name="sOrdForStnCd" size="1" onchange="chkSOrdForStnCd();">
...
%>
                    ...
                    <tr>
                        <td width="24%"><font face="Times New Roman" color="#80080" size="3"><b>Order&nbsp;Status</b></font></td>
                        <td width="2%"><font face="Times New Roman" color="#80080" size="3"><b>:</b></font></td>
                        <td colspan="4" align="left">
                            <select name="sOrdStatus" onchange=""><!--one hidden variable sOrdSts exists -->
                                <option value="">ALL</option>
                                <%
                                            ...
                                            lstOrdSts = hRemote.getOrdSts(sOrdForStnCd);
                                            iterateOuter = lstOrdSts.iterator();
