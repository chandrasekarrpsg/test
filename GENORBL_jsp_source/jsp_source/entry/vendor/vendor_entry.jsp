...
String aposn=request.getParameter("ap") ;
...
if (aposn.equals("add")){
//here q is inserted to test only. remove q after it has been tested 
      if ( ssnNm1.equals("q") ){
...
String	sql="select * from vendor where rowid='"+rowid+"'" ;
...
while(rs.next()){
	...
	appr_name=rs.getString("appr_name");
	electrical=rs.getString("electrical");
    electronics=rs.getString("electronics");
    instrumentation=rs.getString("instrumen");
    mechanical=rs.getString("mechanical");
    gen_ord_sup=rs.getString("gen_ord_sup");
    lab_supply=rs.getString("lab_supply")	;
    address=rs.getString("address");
    status=rs.getInt("status");
    ref=rs.getString("vendor_ref");
	...
	termination_stat=rs.getString("termination_stat");
	termination_remark=rs.getString("termination_remark");
}
...
if (ssnDept1.equals("PLG") ){
		...
		edit_clause="edit_onclick('vendor_entry_submit.jsp?status=0')" ;
		del_clause="del_onclick('vendor_entry_submit.jsp?del=1&status=0','vendor_entry_submit.asp?status=0&ap="+aposn+"')"  ;
}else{
		edit_clause="" ;
		del_clause="" ;
		//it has been added for test purposes only .  remove after testing 
		edit_clause="edit_onclick('vendor_entry_submit.jsp?status=0')" ;
		del_clause="del_onclick('vendor_entry_submit.jsp?del=1&status=0','vendor_entry_submit.asp?status=0&ap="+aposn+"')"  ;
}
...
<TBODY>
    ...
    <tr align="center">
      ...
      value="Close"  onclick="window.close();"></font></td>
    </tr>
  </table>
  </center></div>
</form>
