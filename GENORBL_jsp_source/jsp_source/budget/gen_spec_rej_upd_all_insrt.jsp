...
<%@ page language="java" import="java.sql.*,oracle.jdbc.driver.*,javax.servlet.jsp.tagext.*,javax.servlet.jsp.*,genOrblJavaPackage.*,javax.swing.*,java.io.*,java.util.*; " %>
 ...
 <%
                        ...
                        String sec_under_auth = request.getParameter("sec_under_auth");
                        String sec_under_auth_act = new String("");
                        String consolidation_done = new String("");
...
System.out.println("chkset"+chkset);
System.out.println("seccdset"+seccdset);
System.out.println("rmksset"+rmksset);

if(sec_under_auth.equals("DGM(I)-BBGS")){
   sec_under_auth_act="DGM1";
}

if(sec_under_auth.equals("DGM(II)-BBGS")){
   sec_under_auth_act="DGM2";
}

System.out.println("sec_under_auth_act:"+sec_under_auth_act);
  ...
  ps = mk.con.prepareStatement("select count(*) from bgt_form2b_data_bbgs where stn_cd=? and sec_cd=? and bgt_yr=?");
       ps.setString(1,stn_cd);
       ps.setString(2,sec_under_auth_act);
       ps.setString(3,finyr);
       rs = ps.executeQuery();
       while(rs.next()){
            consolidation_done = rs.getString(1);
       }
       rs.close();
       ps.close();

if(!consolidation_done.equals("0")){

       ps = mk.con.prepareStatement("delete bgt_form2b_data_bbgs where stn_cd=? and sec_cd=? and bgt_yr=?");
       ps.setString(1,stn_cd);
       ps.setString(2,sec_under_auth_act);
       ps.setString(3,finyr);
       update_count1 = ps.executeUpdate();
       ps.close();
       
       ps = mk.con.prepareStatement("delete bgt_position where stn_cd=? and sec_cd=? and bgt_yr=? and bgt_ver=(select max(bgt_ver) from bgt_position where stn_cd=? and sec_cd=? and bgt_yr=?)");
       ps.setString(1,stn_cd);
       ps.setString(2,sec_under_auth_act);
       ps.setString(3,finyr);
       ps.setString(4,stn_cd);
       ps.setString(5,sec_under_auth_act);
...
<b><font face="Book Antiqua" color="#800000" size="3">REJECTION STATUS OF <%=station%> FOR FINANCIAL YEAR <%=finyr1%></font></b>
</label>
...
System.out.println("rmks[z]:"+rmks[z]);
                z=z+1;
          }
...
System.out.println("cur_bgt_level:"+cur_bgt_level);
       ...
       ps = mk.con.prepareStatement("select max(bgt_level) from bgt_level where stn_cd=? and sec_cd=? and bgt_level<? and bgt_yr=?");
       ps.setString(1,stn_cd);
       ps.setString(2,curseccd);
...
System.out.println("prev_bgt_level:"+prev_bgt_level);
       ...
       ps = mk.con.prepareStatement(" select min(bgt_level) from bgt_level "+
                                    " where stn_cd=? and sec_cd=? and bgt_level>'1' and bgt_yr=? ");
       ps.setString(1,stn_cd);
       ps.setString(2,curseccd);
       ps.setString(3,finyr);
       rs = ps.executeQuery();
