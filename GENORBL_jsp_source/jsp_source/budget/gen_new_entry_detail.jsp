...
if(document.f1.mult_exist.value!='0' && document.f1.mult_exist.value!='1'){
  sec=document.f1.Ssec_cd.value;
}else{
  sec=document.f1.sec_cd.value;
}
...
window.close();
window.open("generation_budget_de_new.jsp?"+parameter,"bottom");
}
}
...
<%
  ...
  String orbl_stn_cd                = new String("");
  String mult_exist                 = new String("");
...
try{
       ...
       ps = mk.con.prepareStatement(" select STATION_LONG_DESC,DEPT_SDESC,orbl_station_cd "+ 
       ...
       while(rs.next()){
            ...
            orbl_stn_cd  = rs.getString(3);
       }
       rs.close();
       ps.close();
      
       ps = mk.con.prepareStatement(" select count(*) from ( "+
                                    " select order_dept from ORBL_CD_DEPT "+ 
                                    " where station=? "+
                                    " and login_dept= ?) "+
                                    " where order_dept in (select section_cd "+
	                            " from bgt_cd_section "+
		     		    " where station_cd=decode(?,'016','B','019','C','013','N','014','T','002','S','042','E'))"+
                                    " AND    order_dept  IN (SELECT DISTINCT sec_cd FROM bgt_level "+
		                                           " WHERE bgt_level='1' AND emp_code=? and stn_cd=decode(?,'016','B','019','C','013','N','014','T','002','S','042','E'))");	
       ps.setString(1,orbl_stn_cd);
       ps.setString(2,sec_cd);
       ps.setString(3,orbl_stn_cd);
       ps.setString(4,empcd);
       ps.setString(5,orbl_stn_cd);
       rs = ps.executeQuery();
       while(rs.next()){
            mult_exist  = rs.getString(1);
       }
       rs.close();
       ps.close();
...
if(!mult_exist.equals("1") && !mult_exist.equals("0")){

       ps = mk.con.prepareStatement(" select order_dept from ( "+
                                    " select order_dept from ORBL_CD_DEPT "+ 
                                    " where station=? "+
                                    " and login_dept= ?) "+
                                    " where order_dept in (select section_cd "+
...
<label style="left:50px; top:140px; position: absolute" > <b>ENTRY FOR SECTION : </b>
<select size="1" name="Ssec_cd" style="font-family: Tahoma; font-size: 8pt; color: #800080; font-weight: bold" onkeypress=""/>
<%
while(rs.next()){
%>
...
<%
       ...
       rs=ps.executeQuery();
while(rs.next()){
   sec_cd=rs.getString(1);
}
rs.close();
ps.close();
}
%>
<label style="left:400px; top:200px; position: absolute" >
...
<label style="left:500px; top:200px; position: absolute" >
<input type="button" value="Back" id="back" name="back" style="border: 3px double #003E00; font-family:Bell MT; color:#800000; font-weight:bold; padding-left:4; padding-right:4; padding-top:1; padding-bottom:1; background-color:#FFC68C" onClick="back_prev();">
</label>
<%
}else{
%>
<label style="left:350px; top:110px; position: absolute" >
<b><font face="Book Antiqua" color="red" size="3">Sorry you are not authorised for entry!!!</font></b>
</label>
<label style="left:450px; top:200px; position: absolute" >
<input type="button" value="Back" id="back" name="back" style="border: 3px double #003E00; font-family:Bell MT; color:#800000; font-weight:bold; padding-left:4; padding-right:4; padding-top:1; padding-bottom:1; background-color:#FFC68C" onClick="back_prev();">
</label>
<%
}
...
<input type="hidden" id= "mult_exist" name ="mult_exist" value="<%=mult_exist%>" />
<%
}catch(Exception e){
  e.printStackTrace();
}finally{
