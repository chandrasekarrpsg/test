...
function enable_btn_all(str,str1,str2,str3,str4){
if(str != '0'){
   if(str1 != '0'){
...
function consolidate_data_all_dmy(){
   var parameter='';
   parameter = parameter + '&finyr='+document.f1.finyr1.value;
   parameter = parameter + '&finyr1='+document.f1.finyr.value;
   parameter = parameter + '&empcd='+document.f1.empcd.value;
//alert(parameter);
window.open("gen_all_consolidate_dmy.jsp?"+parameter,"_self");
}
...
function view_data_all(){
   ...
   parameter = parameter + '&empcd='+document.f1.empcd.value;
//alert(parameter);
window.open("gen_div_compl_bgt_menu.jsp?"+parameter,"_self");
}
...
<%
  ...
  String rej_flg_chk       = new String("");
  String rej_flg_chk1      = new String("");
  ...
  String auth_view_spl               = new String("");
  String gen_div_cnsl_exist          = new String("");
  String gen_div_cnsl_auth           = new String("");
...
try{
 ...
 ps = mk.con.prepareStatement("select case when to_number(to_char(trunc(sysdate),'MM')) >= 12  then to_char(add_months(trunc(sysdate),12),'RRRR')||'-'||to_char(add_months(trunc(sysdate),24),'RRRR') else to_char(trunc(sysdate),'RRRR')||'-'||to_char(add_months(trunc(sysdate),12),'RRRR') end from dual ");
       rs = ps.executeQuery();
       while(rs.next()){
...
<input type="button" value="Submit" name="sub" id="sub" style="border:3px double #003300; font-family: Tahoma; font-size: 10pt; color: #800000; font-weight: bold" onclick="if(validate_form()) f1.submit();"/>&nbsp;&nbsp;<input type="button" value="Back" name="back" id="back" style="border:3px double #003300; font-family: Tahoma; font-size: 10pt; color: #800000; font-weight: bold" onclick="back_prev();"/></label></p>

<label style="left:550px; top:50px; position: absolute" >
...
System.out.println("finyr2:"+finyr2);
       ...
       ps = mk.con.prepareStatement("select count(*) from bgt_form2b_data where stn_cd='A' and sec_cd='ALL' and bgt_yr=?");
       ps.setString(1,finyr1);
       rs = ps.executeQuery();
       while(rs.next()){
            gen_div_cnsl_exist = rs.getString(1);
       }
       rs.close();
       ps.close();
       
       ps = mk.con.prepareStatement("select count(*) from bgt_level where emp_code=? and view_all_mkr='Y' and bgt_yr=?");
       ps.setString(1,empcd);
       ps.setString(2,finyr1);
       rs = ps.executeQuery();
       while(rs.next()){
            gen_div_cnsl_auth = rs.getString(1);
       }
       rs.close();
       ps.close();
...
<br>
<%
if(gen_div_cnsl_exist.equals("0") && !gen_div_cnsl_auth.equals("0")){
%>
<table style="width: 100%" border="0">
        <tr>
                <td style="width: 70%"  align="right" size="4"><strong><input type="button" value="Generation Division Consolidated View (INTERIM)" name="DCV" id="DCV" style="border:3px double #003300; font-family: Tahoma; font-size: 10pt; color: #800000; font-weight: bold" onclick="consolidate_data_all_dmy();"/></strong></td>
                <td style="width: 30%"  align="right" size="4"><strong><a href="#" onClick="window.open('gen_budget_appr_rej_hist.jsp? finyr=<%=finyr1%>&finyr1=<%=finyr%>&empcd=<%=empcd%>','_self');">Approval/Rejection History</strong></td>
        </tr>
</table>
<%
}else{
...
<table style="width: 100%" border="0">
        <tr>
                <td style="width: 70%"  colspan="7" align="right" size="4"><strong>&nbsp;</strong></td>
                <td style="width: 30%"  colspan="7" align="right" size="4"><strong><a href="#" onClick="window.open('gen_budget_appr_rej_hist.jsp? finyr=<%=finyr1%>&finyr1=<%=finyr%>&empcd=<%=empcd%>','_self');">Approval/Rejection History</strong></td>
        </tr>
</table>
<%
}
...
<table style="width: 100%" border="1">
        <tr>
                <td style="width: 20%" class="style3"  colspan="7" align="middle"><strong>GENERATION DIVISION BUDGET STATUS</strong></td>
        </tr>
        ...
        <tr>
                <td style="width: 15%" class="style5" align="left"><strong>GENERATION DIVISION</strong></td>
<%
...
<%
       ...
       ps2 = mk.con.prepareStatement("select count(*) from bgt_level where stn_cd in ('B','S','T','N','C','E') and emp_code=? and CONSOLIDATE_MKR='Y' and CONSOLIDATE_STN_CD='A' and bgt_yr=?");
       ...
       ps2 = mk.con.prepareStatement("select count(*) from bgt_level where nvl(view_all_mkr,' ')='Y' and emp_code=? and bgt_yr=? ");
       ps2.setString(1,empcd);
       ps2.setString(2,finyr1);
       rs2 = ps2.executeQuery();
       while(rs2.next()){
            auth_view_spl = rs2.getString(1);
       }
       rs2.close();
       ps2.close();
%>
...
<script>
enable_btn_all(<%=auth_flg1%>,<%=gen_entry_finalize_flg%>,<%=gen_entry_pndg_cmpl_flg%>,<%=gen_entry_compile_flg%>,<%=auth_view_spl%>);
</script>
</table>
<br>
<table style="width: 100%" border="1">
...
if(rs.getString(1).equals("B")){
   ...
   ps1 = mk.con.prepareStatement(" select count(distinct sec_cd)  "+ 
                                 ...
                                 "  where stn_cd = ?  and sec_cd in ('DGM1','DGM2') and bgt_yr = ? ");
       ps1.setString(1,rs.getString(1));
       ps1.setString(2,cur_bgt_yr);
       rs1 = ps1.executeQuery();
while(rs1.next()){
...
ps1.close();
  ...
  if ((rs.getString(1).equals("B") && stn_wait_compile_exist.equals("2")) || (!rs.getString(1).equals("B") && Integer.parseInt(stn_wait_compile_exist) == Integer.parseInt(cnt_stn_sec))){

   ps1 = mk.con.prepareStatement(" select distinct emp_desg from BGT_LEVEL "+ 
                                      "  where stn_cd = ? "+
                                      "  and CONSOLIDATE_MKR = 'Y' AND CONSOLIDATE_STN_CD =? and bgt_yr=? ");
       ps1.setString(1,rs.getString(1));
...
ps1.close();
if(rs.getString(1).equals("B")){
System.out.println("BBGShere");
      rmks="CONSOLIDATION BY THE GM-BBGS PENDING.";
}else{
      rmks="CONSOLIDATION BY THE "+cmpl_auth+" PENDING.";
System.out.println("OTHER BBGShere");
}
      rej_flg      = "1";
      cmpl_flg     = "1";
    ...
    if(rs.getString(1).equals("B")){
          ...
          ps1 = mk.con.prepareStatement(" select count(*) from "+ 
                                               ...
                                               "  from BGT_level "+
                                               "  where stn_cd = ? "+
                                               "  and sec_cd  not in ('ALL','DGM1','DGM2') "+
                                               "  and bgt_yr=? "+
                                               "  group by sec_cd) a, "+
                                               " (select sec_cd "+
                                               "  from BGT_FORM2B_DATA_BBGS "+
...
<%
       ps1 = mk.con.prepareStatement("select distinct (bgt_level) from ( "+
                                                          ...
                                                          " and sec_cd <>'ALL' and bgt_yr=? group by sec_cd) ");
       ps1.setString(1,rs.getString(1));
       ps1.setString(2,finyr1);
       rs1 = ps1.executeQuery();
       while(rs1.next()){
       ...
       ps1 = mk.con.prepareStatement("select count(*) from bgt_level where stn_cd=? and emp_code=? "+ 
                                     ...
                                     " and bgt_level=(select max(bgt_level) from bgt_level "+ 
                                                     " where stn_cd=? and CONSOLIDATE_MKR='Y' "+ 
                                                     " and CONSOLIDATE_STN_CD=? and bgt_yr=?) ");
       ...
       ps1 = mk.con.prepareStatement("select count(*) from bgt_level where emp_code=? and nvl(view_all_mkr,' ')='Y' and bgt_yr=?");
...
if(auth_flg_view.equals("0")){
  if(rs.getString(1).equals("C")){
       ps1 = mk.con.prepareStatement("select count(*) from bgt_level where stn_cd=? and emp_code=? and bgt_yr=?");
       ps1.setString(1,rs.getString(1));
       ps1.setString(2,empcd);
       ps1.setString(3,finyr1);
       rs1 = ps1.executeQuery();
       ...
       ps1 = mk.con.prepareStatement("select count(*) from bgt_level where stn_cd=? and sec_cd<>'CTM' and emp_code=? and bgt_yr=?");
       ps1.setString(1,rs.getString(1));
       ps1.setString(2,empcd);
       ps1.setString(3,finyr1);
       rs1 = ps1.executeQuery();
       ...
       ps1 = mk.con.prepareStatement("select count(*) from bgt_form2b_data where stn_cd=? and sec_cd='ALL' and bgt_yr=?");
       ps1.setString(1,rs.getString(1));
       ps1.setString(2,cur_bgt_yr);
       rs1 = ps1.executeQuery();
       while(rs1.next()){
            rej_flg_chk = rs1.getString(1);
       }
       rs1.close();
       ps1.close();

       ps1 = mk.con.prepareStatement("select count(*) from bgt_form2b_data where stn_cd='A' and sec_cd='ALL' and bgt_yr=?");
       ps1.setString(1,cur_bgt_yr);
       rs1 = ps1.executeQuery();
       while(rs1.next()){
            rej_flg_chk1 = rs1.getString(1);
       }
       rs1.close();
       ps1.close();

if(!rej_flg_chk.equals("0") && rej_flg_chk1.equals("0")){
  rej_flg="1";
}
