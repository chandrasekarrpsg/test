...
if(stn_cd.equals("B")){
       ...
       ps = mk.con.prepareStatement(" select nvl(count(distinct sec_cd),0) "+
                                    ...
                                    " where nvl(publish_mkr,' ')='Y' "+
                                    " and bgt_yr=?  "+
                                    " and sec_cd in (select distinct sec_cd from bgt_level where emp_code=? and stn_cd=? and sec_cd<>'STN' and bgt_yr=? ) ");
       ps.setString(1,finyr);
       ps.setString(2,empcd);
       ps.setString(3,stn_cd);
...
<%
       ...
       ps3 = mk.con.prepareStatement(" select distinct emp_desg from BGT_LEVEL "+
                                     " where STN_CD=? and bgt_level='3' and bgt_yr=? order by emp_desg ");
       ps3.setString(1,stn_cd);
       ps3.setString(2,finyr);
...
if(sec_all_auth.equals("DGM(II)-BBGS")){
  sec_all_auth_act="DGM2";
       ...
       ps = mk.con.prepareStatement(" select SEC_CD from BGT_LEVEL "+
                                     " where STN_CD=? and emp_desg=? and bgt_yr=? ORDER BY sec_cd ");
       ps.setString(1,stn_cd);
...
<%
       ...
       ps1 = mk.con.prepareStatement(" select count(*) from BGT_FORM2B_DATA_BBGS "+
                                     ...
                                     " and   sec_cd in (select distinct sec_cd from bgt_level where emp_desg=? and bgt_yr=?) "+ 
                                     " and   nvl(bgt_fnl,' ')='F' ");
  ...
  sec_all_auth_act="DGM2";
      ...
      if(!sec_bgt_ready_all.equals("0")){
          ...
          ps1 = mk.con.prepareStatement(" select count(distinct sec_cd) "+
                                        " from bgt_level where emp_desg=? and bgt_yr=? ");
          ps1.setString(1,sec_all_auth);
          ps1.setString(2,finyr);
          rs1 = ps1.executeQuery();
          while(rs1.next()){
          ...
          ps1 = mk.con.prepareStatement(" select count(distinct sec_cd) from bgt_form2b_data_bbgs "+
                                        " where bgt_yr=? and sec_cd in (select distinct sec_cd from bgt_level where emp_desg=? and bgt_yr=?) "+
                                        " and bgt_level=? ");
          ps1.setString(1,cur_bgt_yr);
          ps1.setString(2,sec_all_auth);
          ps1.setString(3,finyr);
          ps1.setString(4,consolidate_all_bgt_lvl);
          rs1 = ps1.executeQuery();
          while(rs1.next()){
    ...
    if(!sec_bgt_finalized.equals("0")){
      ...
      rmks = "BUDGET FINALIZED.";
       ...
       ps1 = mk.con.prepareStatement(" select max(bgt_level) from BGT_LEVEL "+
                                     " where stn_cd =? and nvl(consolidate_mkr,' ') = 'Y' and sec_cd=? and bgt_yr=? ");
       ps1.setString(1,stn_cd);
              ...
              rmks              = "SECTION BUDGET READY";
                ...
                if(!sec_bgt_pndg_apprvl.equals("0")){
                     ps1 = mk.con.prepareStatement(" select EMP_DESG from BGT_LEVEL "+
                                                   " where stn_cd =? and sec_cd=? and bgt_yr=?  and bgt_level = (select distinct bgt_level from BGT_FORM2B_DATA_BBGS where stn_cd=? and sec_cd=? and bgt_yr=?)");
                     ps1.setString(1,stn_cd);
                     ps1.setString(2,rs.getString(1));
                     ps1.setString(3,finyr);
                   ...
                   ps1 = mk.con.prepareStatement(" select distinct emp_desg FROM BGT_LEVEL "+
                                                 " where stn_cd =?  and sec_cd=?  and bgt_level= '1' and bgt_yr=? and emp_code=(select distinct upd_by from bgt_form2b_data_bbgs where stn_cd=? and sec_cd=? and bgt_yr=? and crt_by <>'IT') ");
                   ps1.setString(1,stn_cd);
                   ps1.setString(2,rs.getString(1));
                   ps1.setString(3,finyr);
                   ps1.setString(4,stn_cd);
...
if(!forward_exist.equals("0")){
   forward_flg="1";
       ...
       ps1.setString(4,finyr);
       ps1.setString(5,stn_cd);
       ps1.setString(6,rs.getString(1));
       ps1.setString(7,finyr);
       rs1 = ps1.executeQuery();
while(rs1.next()){
...
ps3.close();
       ...
       ps1 = mk.con.prepareStatement(" select emp_desg from BGT_LEVEL "+
                                     " where stn_cd =?  and nvl(consolidate_mkr,' ') ='Y' and nvl(consolidate_stn_cd,' ') =? and bgt_yr=? ");
       ps1.setString(1,stn_cd);
        ...
        ps1.close();
           if(!sec_bgt_ready.equals("0")){
              ...
              rmks              = "SECTION BUDGET READY";
                ...
                if(!sec_bgt_pndg_apprvl.equals("0")){
                     ps1 = mk.con.prepareStatement(" select EMP_DESG from BGT_LEVEL "+
                                                   " where stn_cd =? and sec_cd=? and bgt_yr=?  and bgt_level = (select distinct bgt_level from BGT_FORM2B_DATA where stn_cd=? and sec_cd=? and bgt_yr=?)");
                     ps1.setString(1,stn_cd);
                     ps1.setString(2,rs.getString(1));
                     ps1.setString(3,finyr);
                     ps1.setString(4,stn_cd);
                   ...
                   ps1.close();
                    if(!sec_bgt_pub.equals("0")){
                   ps1 = mk.con.prepareStatement(" select distinct emp_desg FROM BGT_LEVEL "+
                                                 " where stn_cd =?  and sec_cd=?  and bgt_level= '1' and bgt_yr=? and emp_code=(select distinct upd_by from bgt_form2b_data where stn_cd=? and sec_cd=? and bgt_yr=? and crt_by <>'IT') ");
                   ps1.setString(1,stn_cd);
                   ps1.setString(2,rs.getString(1));
                   ps1.setString(3,finyr);
                   ps1.setString(4,stn_cd);
