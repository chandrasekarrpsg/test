...
<form name = "BACK" target="_self" action="budget_dash_main_dtl.jsp" method="POST" />
 <input type="hidden"  name = "sec_cd"    value="" />
 <input type="hidden"  name = "stn_cd"    value="" />
...
/** ForceFully icnt1 assigned non zero value ***/
     icnt1 = 1;
...
/**** New Code Start ***/
                  ...
                  idec_plc_ctm = rs1.getInt(13);/** DEC PLACES AFTER IN CTM **/
                  idec_plc_stn = rs1.getInt(14);/** DEC PLACES AFTER IN STN **/

                  rows.add(rs1.getString(13)) ;/** DEC PLACES AFTER IN CTM **/
                  rows.add(rs1.getString(14)) ;/** DEC PLACES AFTER IN STN **/ 
                  l1.add(rows);
            ...
            if ( readOnlyMkr== 'T' ) {
                for ( j=2; j<9; j++ ) {
                    ...
                    if(myData[i][j].trim()=='0.000'){
                       ...
                       var i_dec_plc_stn = myData[i][20];
                       var i_dec_plc_ctm = myData[i][19];

                       if(j==7){
                          //alert("i_dec_plc_ctm "+i_dec_plc_ctm);
                          obj.setCellText(trunc_no(myData[i][j],i_dec_plc_ctm),j,i);
                       }
                       else{
                          //alert("i_dec_plc_stn "+i_dec_plc_stn);
                          obj.setCellText(trunc_no(myData[i][j],i_dec_plc_stn),j,i);
                       }
                    }
