...
if(document.f1.stn_cd.value=='B'){
  window.open("generation_budget_de_bbgs_temp.jsp?"+parameter,"_self");
}else{
  window.open("generation_budget_de_new_temp.jsp?"+parameter,"_self");
}
...
System.out.println("empcd:"+empcd);
       ...
       ps = mk.con.prepareStatement("select distinct sec_cd from bgt_level where emp_code=? and stn_cd=? and bgt_yr=? ");
       ps.setString(1,empcd);
       ps.setString(2,stn_cd);
       ps.setString(3,finyr);
       rs = ps.executeQuery();
       while(rs.next()){
            sec_cd = sec_cd+"#"+rs.getString(1);
       }
       rs.close();
       ps.close();
sec_cd = sec_cd.substring(1);
System.out.println("sec_cd:"+sec_cd);

                             cstmt = mk.con.prepareCall( "{call DPD_POPL_CONSOLIDATED_VIEW(?,?,?,?)}");
                             ...
                             System.out.println("DPD_POPL_CONSOLIDATED_VIEW PROCEDURE CALLED");
                             ...
                             if(iRet==1){
                                ...
                                sMssg = "CONSOLIDATION POPULATION DONE SUCCESSFULLY FOR "+station;
                                ...
                                if(iRet==0){
                                   ...
                                   iFailureFlag = 1;
                                   sMssg = "ERROR OCCURRED WHILE CONSOLIDATION POPULATION OF "+station;
...
%>
<label style="left:5px; top:100px; position:absolute;"><b>
<font face="Tahoma" color="red">DATA PREPARATION OF CONSOLIDATED VIEW FOR SECTION(S) UNDER : <%=cur_emp_desg%> OF STATION : <%=station%> FOR FINANCIAL YEAR : <%=finyr1%> FAILED!!!!!!! </font></b></label>
<br>
<br>
<label style="left:250px; top:150px; position:absolute;"><b>
