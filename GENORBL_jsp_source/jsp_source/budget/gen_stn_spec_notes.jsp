...
if(sec_cd.equals("##")){
   sec_cd_spl="ALL SECTIONS INTERIM";
   sec_cd="ALL";
   section="ALL INTERIM [ "+empcd+" ] ";
}
...
<%
 ...
 if(sec_cd_spl.equals("ALL SECTIONS INTERIM")){
%>
                <td style="width: 20%"  colspan="7" align="right" size="4"><strong><a href="#" onClick="window.open('generation_budget_de_ovral_cnsl_temp.jsp?stn_cd=<%=stn_cd%>&T1=<%=finyr%>&emp_desg=<%=your_desg%>&username=<%=empcd%>&bgt_lvl=2&stn_long_desc=<%=station%>','_self');">Back</strong></font></b></strong></td>
<%
 }else{
%>
...
<%
}else if(sec_cd.equals("ALL") && sec_cd_spl.equals("ALL SECTIONS INTERIM")){
%>
                <td style="width: 20%"  colspan="7" align="right" size="4"><strong><a href="#" onClick="window.open('generation_budget_de_ovral_cnsl_temp.jsp?stn_cd=<%=stn_cd%>&T1=<%=finyr%>&emp_desg=<%=your_desg%>&username=<%=empcd%>&bgt_lvl=2&stn_long_desc=<%=station%>','_self');">Back</strong></font></b></strong></td>
<%
}else{
