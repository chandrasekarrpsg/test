...
<b><font face="Book Antiqua" color="#800000" size="3">GENERATION BUDGET HISTORY FOR FINANCIAL YEAR <%=finyr1%></font></b>
</label>
...
<table style="width: 100%" border="0">
        <tr>
                <td style="width: 20%"  colspan="7" align="right" size="4"><strong><a href="#" onClick="window.open('budget_dash_main_dtl.jsp?finyr1=<%=finyrst%>&finyr2=<%=finyrend%>','_self');">Back</strong></font></b></strong></td>
        </tr>
</table>
...
<%
       ...
       ps = mk.con.prepareStatement(" select count(*) from BGT_FORM2B_DATA_ARC "+ 
                                    " where stn_cd='A'  and sec_cd='ALL' and bgt_yr=?");
       ps.setString(1,finyr);
       rs = ps.executeQuery();
       while(rs.next()){
...
if(!entry_consolidated_exist.equals("0") || !entry_consolidated_exist1.equals("0")){
       ...
       ps = mk.con.prepareStatement(" select * from ( "+
                                    ...
                                    " from bgt_form2b_data where stn_cd='A' and sec_cd='ALL' "+ 
                                    " and bgt_yr= ? and bgt_level='5' "+ 
                                    " union all "+
                                    " select distinct to_char(crt_dt,'dd/mm/yyyy'),crt_by,bgt_ver "+ 
                                    " from bgt_form2b_data_arc "+ 
                                    " where stn_cd='A' "+ 
                                    " and sec_cd='ALL' and bgt_yr=? and bgt_level='5' ) "+
                                    " order by bgt_ver ");
       ps.setString(1,finyr);
...
<%
       ps = mk.con.prepareStatement(" select count(distinct bgt_ver) from BGT_POSITION "+ 
                                    " where stn_cd in ('B','T','N','S','C','E') and sec_cd='ALL' and bgt_yr=? and bgt_level='5' ");
       ps.setString(1,finyr);
       rs = ps.executeQuery();
       while(rs.next()){
...
if(!app_hist_exist.equals("0")){
    ...
    ps = mk.con.prepareStatement(" select bgt_ver,decode(stn_cd,'B','BBGS','T','TGS','S','SGS','N','NCGS','C','CTM','E','EDGE'),decode(SEND_TO_LOWER_LEVEL_DT,'','ACCEPTED','REJECTED') status, "+
                                 " nvl(to_char(SEND_TO_LOWER_LEVEL_DT,'dd/mm/yyyy'),'&nbsp'),nvl(SEND_TO_LOWER_LEVEL_BY,'&nbsp'),nvl(remarks,'&nbsp') "+ 
                                 " from bgt_position "+
                                 " where stn_cd in ('B','T','N','S','C','E') and sec_cd='ALL' "+ 
                                 " and bgt_yr=? and bgt_level='5' order by bgt_ver,decode(stn_cd,'B','1','T','2','S','3','N','4','C','5','E','6') ");
       ps.setString(1,finyr);
       rs = ps.executeQuery();
%>
<table style="width: 100%" border="1">
        ...
        <tr>
                ...
                <td style="width: 20%" class="style3"><strong>STATION </strong></td>
...
<%
while(rs.next()){
      ps1 = mk.con.prepareStatement(" select nvl(max(emp_desg),'&nbsp') from bgt_level where emp_code=? and bgt_yr=? ");
