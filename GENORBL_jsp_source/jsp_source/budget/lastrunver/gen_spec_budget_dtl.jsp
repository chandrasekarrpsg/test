...
if(document.f1.stn_cd.value=='B'){
  ...
  window.open("generation_budget_de.jsp?"+parameter,"_self");
}
...
<table style="width: 100%" border="0">
        <tr>
                <td style="width: 20%"  colspan="7" align="right" size="4"><strong><a href="#" onClick="window.open('gen_overall_bbgs_appr_rej_hist.jsp? finyr=<%=finyr%>&finyr1=<%=finyr1%>&empcd=<%=empcd%>','_blank');">Approval/Rejection History</strong></td>
        </tr>
</table>
<br>
<%
...
while(rs3.next()){
System.out.println("sec_all_auth:"+rs3.getString(1));
  
sec_all_auth = rs3.getString(1);
System.out.println("sec_all_auth:"+sec_all_auth);
...
%>
<tr>
                <td style="width: 30%" class="style5" align="center"><strong><a href="#" onClick="window.open('sec_bgt_hist.jsp?stn_cd=<%=stn_cd%>&sec_cd=<%=rs.getString(1)%>&finyr=<%=finyr%>&finyr1=<%=finyr1%>&empcd=<%=empcd%>','_blank');"><%=rs.getString(1)%></strong></td>
<%
       ps1 = mk.con.prepareStatement(" select count(*) from BGT_FORM2B_DATA_BBGS "+
...
<script>
enable_btn(<%=auth_flg%>,<%=forward_flg%>,<%=rej_flg%>,<%=view_flg%>,<%=view_auth_flg%>,<%=cnt%>);
</script>
