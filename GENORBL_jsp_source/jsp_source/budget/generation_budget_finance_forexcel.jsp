...
function trunc_no_1(number,plc_aft_dec,obj_id){
...
<%
  ...
  String sBgtCTM               = new String("");
  String sBgtSTN               = new String("");
...
<table style="width: 100%" class="style1">
	<tr>
		<td class="style3" colspan="7" align="left"><strong><u>REVENUE BUDGET( FINANCE FORMAT <%= sBgtHdr %>)-F.Y. : <%= sBgt_Fr_Year %> - <%= sBgt_To_Year %> </u></strong></td>
...
/**** New Code Start ***/
                  ...
                  rows.add(rs1.getString(13)) ;/** NO_DEC_PLACES_CTM **/
                  rows.add(rs1.getString(14)) ;/** NO_DEC_PLACES_STN **/
           ...
           ps1=mk.con.prepareStatement("SELECT B.BGT_SEQ,B.SL_NO_ROWORDER,B.BGT_HD_ROWORDER, "
                                     ...
                                     + " DECODE(TRIM(TO_CHAR(NVL(A.AMT_STN_BBGS_TGS_SGS_NCGS,0),'99990.999')),'0.000',' ',SUBSTR(TO_CHAR(NVL(A.AMT_STN_BBGS_TGS_SGS_NCGS,0),'99990.999'),0,6+DECODE(B.NO_DEC_PLACES_STN,0,B.NO_DEC_PLACES_STN,B.NO_DEC_PLACES_STN+1))) all_stn_tot,"
                                     + " DECODE(TRIM(TO_CHAR(NVL(A.AMT_CTM_EDGE,0),'99990.999')),'0.000',' ',SUBSTR(TO_CHAR(NVL(A.AMT_CTM_EDGE,0),'99990.999'),0,6+DECODE(B.NO_DEC_PLACES_CTM,0,B.NO_DEC_PLACES_CTM,B.NO_DEC_PLACES_CTM+1))) ctm_edge_tot,"
...
<%
        ...
        for( i=0; i <detailList.size(); i++){
             ...
             sBgtCTM = (String)rows.get(19);
             sBgtSTN = (String)rows.get(20);
             ...
             System.out.println("sBgtCTM:"+i+","+sBgtCTM);
             System.out.println("sBgtSTN:"+i+","+sBgtSTN);
/*
...
%>
  ...
  <td class="style35" id="T<%= icnt %>" style=background-color:<%= sAmtBColor %>;color:<%= sAmtFColor %>;<%= sItalics %>><%= sAmt %></td>
