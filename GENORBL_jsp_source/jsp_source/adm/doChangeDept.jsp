    ...
    function info(){
     ...
     <body background="Back11.jpg" onload="cal();">
           ...
           <div class="left">
	        ...
	        <label style="POSITION: absolute; LEFT: 700px; top:575px"  class="style5" ><b>2012 © Developed by I.T. Department</b></label>
...
<%  
    try{ // loop1 
         ...
         try{ //loop2
              ...
              if (empcode != null) { //loop3 
                  ps0=mk.con.prepareStatement(" SELECT COUNT(*)    "+
                                              " FROM ADM_EMP_MST   "+ 
                                              " WHERE  emp_code=?  ",
                                          ResultSet.TYPE_SCROLL_INSENSITIVE,
                                          ResultSet.CONCUR_READ_ONLY );
                  ps0.setString(1,empcode);
                  rs0=ps0.executeQuery();
                  while (rs0.next()) {   //loop4
                         cnt=rs0.getInt(1);
                         if (cnt == 0) {  //loop5
                             empfound = 0;
                         } //loop5 ends here
         ...
         ps1  = mk.con.prepareStatement(" SELECT TRIM(emp_fname)||' '||TRIM(emp_lname), "+
                                        ...
                                        " AND SUBSTR(ADM_EMP_MST.emp_dept,1,2) = SUBSTR(ORBL_CD_DEPT_MST.dept,1,2) ",
                                        ResultSet.TYPE_SCROLL_INSENSITIVE,
                                        ResultSet.CONCUR_READ_ONLY );
...
<%
                       ...
                       if (upddept != null && upddept.length()>1) {  //loop5
                           ps2 = mk.con.prepareStatement(" UPDATE ADM_EMP_MST   "+
                                                         " SET    emp_dept = ?  "+
                                                         " WHERE  emp_code = ?  ");
                           ps2.setString(1,upddept);
...
%>
                   <label style="left: 50px;top: 360px;position: absolute">
                          <font color="red" size="6"> 
                                <p align="center"> No record found with given employee code</p> 
                          </font>
                   </label>
<%
                       } //loop5 ends here
                  } //loop4 ends here
              } // loop3 ends here (empcode != null)
         } // loop2 
