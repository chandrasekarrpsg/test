...
try{
        ...
        letter_ref = request.getParameter ("refno");
...
if(letter_month.equals("01")){
   ...
   letter_month = letter_month;
        ...
        ps=mk.con.prepareStatement(query);
        ps.setString(1,letter_month);
        ps.setString(2,letter_yr);
        rs = ps.executeQuery();

        while (rs.next()) {
            case_exist = rs.getString (1);
        }
        rs.close ();
        ps.close ();
...
if(letter_exist.equals("0")){
        ...
        //insert letter referrence to database (table: IDCMS_LET_REF)
         ps = mk.con.prepareStatement(" INSERT INTO IDCMS_LET_REF(CONTEST_MONTH,CONTEST_YEAR,LETTER_REF, "+
                                      " LETTER_DT,LETTER_PRINT_DT,CRT_BY,CRT_DT) "+ 
                                      " VALUES ( ?,?,?,?,?,?,sysdate) ");
         ps.setString(1,letter_month); 
         ps.setString(2,letter_yr); 
         ps.setString(3,letter_ref); 
         ps.setString(4,date); 
         ps.setString(5,date); 
         ps.setString(6,name); 
         rs = ps.executeQuery();
...
while (rs.next ()) {
     ...
     if (count1+count2+count3 > 0) {
        ...
        <div class="body">
            ...
            the above sum of <b><%=sum%>/-</b> Payable at <%=office_name%>, on account of reimbursement of prize 
...
%>
 <TABLE height=20% width="100%" border=0 cellspacing="0" style="background-color: #77FFFF;">
        <TBODY>
          <TR>
            <TD align=middle width=100% height=100% ><font color="#cc3300" face="Courier New" Size=5><b>Letter Have Been Already Generated For <%=monthname%>, <%=letter_yr%>. Please Use Reprint Option.</b></font></TD>
          </TR>
          <TR>
...
%>
 <TABLE height=20% width="100%" border=0 cellspacing="0" style="background-color: #77FFFF;">
        <TBODY>
          <TR>
            <TD align=middle width=100% height=100% ><font color="#cc3300" face="Courier New" Size=5><b>No Cases Have Been Matured Till Award For <%=monthname%>, <%=letter_yr%>.</b></font></TD>
          </TR>
          <TR>
            <TD align=middle width=100% height=100% ><input type="button" value="CLOSE" name="close" onClick="window.close();" ></b></font></TD>
          </TR>
