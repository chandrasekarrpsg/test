...
function search(sLoginStnCd,sLoginStnShrtNm,sLoginStnLongNm,sLoginDeptCd,sLoginDeptShrtNm,sLoginDeptLongNm,sLoginEmpCd,sLoginEmpNm,sLoginEmpDesg,sLoginEmpLvl,sLoginAdmMkr,sLoginEmpKaizenMkr,sContestId,sMode,sMsg){
var url="ipicms_nomEntry.jsp?sLoginStnCd="+sLoginStnCd+"&sLoginStnShrtNm="+sLoginStnShrtNm+"&sLoginStnLongNm="+sLoginStnLongNm+"&sLoginDeptCd="+sLoginDeptCd+"&sLoginDeptShrtNm="+sLoginDeptShrtNm+"&sLoginDeptLongNm="+sLoginDeptLongNm+"&sLoginEmpCd="+sLoginEmpCd+"&sLoginEmpNm="+sLoginEmpNm+"&sLoginEmpDesg="+sLoginEmpDesg+"&sLoginEmpLvl="+sLoginEmpLvl+"&sLoginAdmMkr="+sLoginAdmMkr+"&sLoginEmpKaizenMkr="+sLoginEmpKaizenMkr+"&sContestId="+sContestId+"&sMode="+sMode+"&sMsg="+sMsg
            window.status="Opening Nomination Details Screen !. Please wait....................."
...
try{
        ...
        int count        = 0;
        boolean isand    = false;
        String month     = new String (request.getParameter ("m"));
        String year      = new String (request.getParameter ("sYr"));
        ...
        String cat_prm   = new String (request.getParameter ("cat"));
        String section   = new String (request.getParameter ("sec"));
...
System.out.println("cat_prm:"+cat_prm);
System.out.println("section:"+section);

        query = "SELECT count(*) FROM IDCMS_MST WHERE CONTEST_MONTH =? and CONTEST_YEAR=? and STATION like decode(?,'all','%',?) and STAGE like decode(?,'all','%',?) and category_type =? and kaizen_for_dept=trim(?) ";
        ps=mk.con.prepareStatement(query);
...
<%
        query = "SELECT CONTEST_ID_FULL,STAGE,NVL(TITLE,'&nbsp'),nvl(CATEGORY_TYPE,'X'),proposed_by,dept,station,station_sht_desc,contest_id FROM IDCMS_MST a,cd_station b where a.station=b.orbl_station_cd and CONTEST_MONTH =? and CONTEST_YEAR=? and STATION like decode(?,'all','%',?) and STAGE like decode(?,'all','%',?) and category_type =? and kaizen_for_dept=trim(?) order by decode(station_sht_desc,'BBGS','1','TGS','2','SGS','S','NCGS','N','CTM','C','EDGE','E'),proposal_date,a.contest_id";
        ps=mk.con.prepareStatement(query);
        ps.setString(1,month);
        ps.setString(2,year);
        ps.setString(3,office);
...
<tr>
              ...
              <td style="width: 20%" class="<%=sStyle%>"><strong><input type="button" value="view details" onClick="search('<%= sLoginStnCd%>','<%= sLoginStnShrtNm%>','<%= sLoginStnLongNm%>','<%= sLoginDeptCd%>','<%= sLoginDeptShrtNm%>','<%= sLoginDeptLongNm%>','<%= sLoginEmpCd%>','<%= sLoginEmpNm%>','<%= sLoginEmpDesg%>','<%= sLoginEmpLvl%>','<%= sLoginAdmMkr%>','<%= sLoginEmpKaizenMkr%>','<%= contestid_act%>','M','');" />
              </strong></td>
      </tr>
