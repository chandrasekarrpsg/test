...
if(isset($_REQUEST['pwdsubmit']) && ($_REQUEST['pwdsubmit']=='Submit'))
{
	...
	if($passmessage=='') {
		if(check_session('admin'))
		{
			...
			{
				$query="update ignore ".TABLE_PREFIX."administrator set adminpassword ='".g_db_input(md5($new1))."' where id='".$_SESSION['admin']['id']."'";
	...
	if($admin_email!='')
	{
		$query="update ignore ".TABLE_PREFIX."administrator set admin_email ='".$admin_email."' where id='".$_SESSION['admin']['id']."'";
				$res=g_db_query($query);
		if($res)
		{
