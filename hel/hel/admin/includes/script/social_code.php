...
if(isset($_REQUEST['save_social']) && $_REQUEST['save_social'] == 'submit'){
 $edit_social_sql="UPDATE ignore tbl_social set
			...
			social_dribble ='".$_REQUEST['social_dribble']."',
			social_pinterest ='".$_REQUEST['social_pinterest']."',
			social_google ='".$_REQUEST['social_google']."'			
            where social_id = 1";		
$edit_social_row=mysqli_query($conn,$edit_social_sql)  or die(mysqli_error($conn));
if($edit_social_row){
$messages.= showMessage("Social has been updated successfully",'success');		
}
else
{
	$messages.= "";
